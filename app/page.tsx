...
export default function Home() {
  ...
  useEffect(() => {
    const fetchDiscs = async () => {
      ...
      try {
        const { data, error } = await supabase.from("discs").select("*").order("created_at", { ascending: false })
    ...
    <ProtectedRoute>
      <main className="min-h-screen bg-gray-100">
        ...
        <div className="container mx-auto px-4 py-8">
          <h1 className="text-3xl font-bold mb-4">My Disc Golf Collection</h1>
              ...
              {discs.map((disc) => (
                <Card key={disc.id}>
                  ...
                  <CardContent>
                    ...
                    {disc.for_sale && (
                      <p>
                        <strong>Price:</strong> ${disc.price}
                      </p>
                    )}
                  </CardContent>
                </Card>
